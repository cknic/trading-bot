...
def main():
    ...
    while True:
        ...
        try:
            # ---------------------------------------------------
            # NEW: Update Risk Engine with Open Positions Count
            # ---------------------------------------------------
            # We must tell the risk engine how many positions we hold
            # so it passes the 'fail-closed' check.
            current_open_pos_count = 0
            for pk in pair_keys:
                # Check ledger for each configured pair
                pos_data = get_position(pk)
                if pos_data.get("has_position"):
                    current_open_pos_count += 1
