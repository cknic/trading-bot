...
class RiskEngine:
    ...
    # Core gating
    # -----------------
    def can_trade(self, notional_usd: float, mode: str, pair: Optional[str] = None, side: Optional[str] = None) -> RiskDecision:
        """
        Check if a trade is allowed.
        
        Args:
            notional_usd: Trade size in USD
            mode: "live", "paper", or "dry_run"
            pair: Trading pair/symbol (optional)
            side: "buy" or "sell" (optional) - SELL orders skip position limits
        """
        self._roll_day_if_needed()
        
        # Kill switch always blocks everything
        ...
        # Determine if this is a sell/close order
        is_sell = (side or "").lower() in ("sell", "close")
        
        # Position limit check - SKIP FOR SELL ORDERS (they reduce exposure)
        if self.max_open_positions > 0 and not is_sell:
        ...
        # Notional limit - SKIP FOR SELL ORDERS (we want to close full positions)
        if not is_sell and notional_usd > (self.max_notional_usd_per_trade + 0.05):
            return RiskDecision(
                False,
        ...
        # Daily trade limits still apply to both buys and sells
    ...
    def check(self, side: str, volume: float, price: float) -> bool:
        ...
        # Pass side to can_trade so sells aren't blocked by position limits
        decision = self.can_trade(notional, mode="live", side=side)
